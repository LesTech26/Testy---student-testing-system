...
$admin_city = $currentAdmin['city'] ?? null;
    ...
    switch ($_GET['action']) {
        case 'create':
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if ($is_super_admin) {
                    // Супер-админ может выбрать город или оставить пустым (для всех городов)
                    $city = !empty($_POST['city']) ? trim($_POST['city']) : null;
                } else {
                    // Городской админ автоматически привязывает тест к своему городу
                    $city = $admin_city;
                }

                // Сохраняем тест с учетом города
                $stmt = $pdo->prepare("INSERT INTO tests (title, description, is_active, time_limit, passing_score, grading_system, course_id, specialty_id, city) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$title, $description, $is_active, $time_limit, $passing_score, $grading_system, $course_id, $specialty_id, $city]);
                ...
                if (function_exists('logAdminAction')) {
                    logAdminAction($pdo, 'create_test', "Создан тест ID: $test_id, город: " . ($city ?: 'все города'));
                }
        ...
        case 'edit':
            ...
            // Проверяем доступ к тесту для городского администратора
            if (!$is_super_admin) {
                $stmt = $pdo->prepare("SELECT city FROM tests WHERE id = ?");
                $stmt->execute([$id]);
                $testCity = $stmt->fetchColumn();
                
                // Городской админ может редактировать только тесты своего города или общие тесты (city IS NULL)
                if ($testCity && $testCity !== $admin_city) {
                    $_SESSION['error'] = 'У вас нет прав для редактирования этого теста';
                    header('Location: tests.php');
                    exit();
                }
            }
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                // Обновляем город только если супер-админ
                if ($is_super_admin) {
                    $city = !empty($_POST['city']) ? trim($_POST['city']) : null;
                    $stmt = $pdo->prepare("UPDATE tests SET title = ?, description = ?, is_active = ?, time_limit = ?, passing_score = ?, grading_system = ?, course_id = ?, specialty_id = ?, city = ? WHERE id = ?");
                    $stmt->execute([$title, $description, $is_active, $time_limit, $passing_score, $grading_system, $course_id, $specialty_id, $city, $id]);
                } else {
                    $stmt = $pdo->prepare("UPDATE tests SET title = ?, description = ?, is_active = ?, time_limit = ?, passing_score = ?, grading_system = ?, course_id = ?, specialty_id = ? WHERE id = ?");
                    $stmt->execute([$title, $description, $is_active, $time_limit, $passing_score, $grading_system, $course_id, $specialty_id, $id]);
                }
                ...
                if (function_exists('logAdminAction')) {
                    logAdminAction($pdo, 'edit_test', "Отредактирован тест ID: $id");
                }
        ...
        case 'grading_rules':
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if ($stmt->execute([$grading_rules, $id])) {
                    if (function_exists('logAdminAction')) {
                        logAdminAction($pdo, 'update_grading_rules', "Обновлены правила оценивания для теста ID: $id");
                    }
        ...
        case 'delete':
            if (isset($_GET['id'])) {
                ...
                if (!$is_super_admin) {
                    ...
                    if ($testCity && $testCity !== $admin_city) {
                        $_SESSION['error'] = 'У вас нет прав для удаления этого теста';
                        header('Location: tests.php');
                        exit();
                    }
                }
                ...
                if (function_exists('logAdminAction')) {
                    logAdminAction($pdo, 'delete_test', "Удален тест ID: $id");
                }
        ...
        case 'toggle':
            if (isset($_GET['id'])) {
                ...
                if (!$is_super_admin) {
                    ...
                    if ($testCity && $testCity !== $admin_city) {
                        $_SESSION['error'] = 'У вас нет прав для изменения статуса этого теста';
                        header('Location: tests.php');
                        exit();
                    }
                }
                ...
                if (function_exists('logAdminAction')) {
                    logAdminAction($pdo, 'toggle_test', "Изменен статус теста ID: $id");
                }
...
// Получение всех тестов с информацией о курсе и специальности с учетом прав доступа
if ($is_super_admin) {
    // Супер-админ видит все тесты
    $stmt = $pdo->prepare("
        SELECT t.*, 
    ...
    // Городской админ видит только тесты своего города и общие тесты (city IS NULL)
    $stmt = $pdo->prepare("
        SELECT t.*, 
               s.name as specialty_name,
               (SELECT COUNT(*) FROM questions q WHERE q.test_id = t.id) as question_count 
        FROM tests t 
        LEFT JOIN specialties s ON t.specialty_id = s.id 
        WHERE t.city = ? OR t.city IS NULL
        ORDER BY t.created_at DESC
    ");
...
<body>
    <div class="container">
        ...
        <div class="header mb-4 <?php echo $is_super_admin ? 'super-admin' : 'city-admin'; ?>">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1><i class="bi bi-journal-text"></i> Управление тестами</h1>
                    <p class="mb-0">
                        Администратор: <?php echo htmlspecialchars($_SESSION['admin_username']); ?>
                        ...
                        <?php if ($is_super_admin): ?>
                            <span class="badge bg-warning text-dark ms-2">Супер-админ</span>
                        <?php endif; ?>
                    </p>
                </div>
                <div>
        ...
        <!-- Информация для городского администратора -->
        <?php if (!$is_super_admin && $admin_city): ?>
            <div class="city-info">
                <i class="bi bi-info-circle-fill text-info me-2"></i>
                <strong>Вы создаете тесты для города <?php echo htmlspecialchars($admin_city); ?>.</strong>
                Тесты будут доступны только студентам и преподавателям этого города.
            </div>
        <?php endif; ?>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form method="POST">
                                ...
                                <!-- Выбор города (только для супер-админа) -->
                                <?php if ($is_super_admin): ?>
                                <div class="mb-3">
                                    <label for="city" class="form-label">Город:</label>
                                    <input type="text" class="form-control" id="city" name="city" 
                                        placeholder="Оставьте пустым для тестов, доступных во всех городах"
                                        value="<?php echo isset($test) && !empty($test['city']) ? escape($test['city']) : ''; ?>">
                                    <small class="text-muted">
                                        <i class="bi bi-info-circle"></i> 
                                        Если указать город, тест будет доступен только студентам и преподавателям этого города.<br>
                                        Если оставить пустым - тест будет доступен во всех городах.
                                    </small>
                                </div>
                                <?php else: ?>
                                    <!-- Для городского администратора город подставляется автоматически -->
                                    <input type="hidden" name="city" value="<?php echo htmlspecialchars($admin_city); ?>">
                                    <div class="alert alert-info">
                                        <i class="bi bi-info-circle"></i>
                                        Тест будет создан для города <strong><?php echo htmlspecialchars($admin_city); ?></strong>.
                                        Он будет доступен только студентам и преподавателям этого города.
                                    </div>
                                <?php endif; ?>
